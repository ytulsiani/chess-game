...
public class BoardView {
    ...
    private EventHandler<? super MouseEvent> tileListener(Tile tile) {
        return event -> {
            ...
            // DONE: call firstClick or secondClick, depending on which it is
            //tile.highlight(Color.BLUE);
            if (!clickStatus) {
                firstClick(tile);
                clickStatus = true;
                startingLocation = tile;
            } else {
                if (startingLocation.equals(tile)) {
                    Tile[][] tiling = getTiles();
                    for (Tile[] t : tiling) {
                        for (Tile tt : t) {
                            tt.clear();
                            tt.setSymbol(controller.
                                getSymbolForPieceAt(tt.getPosition()));
                        }
                    }
    ...
    private void firstClick(Tile tile) {
        Set<Move> moveLocations = controller.getMovesForPieceAt(
            tile.getPosition());
        for (Move m : moveLocations) {
            Position tileStart = m.getStart();
            Tile tileStarting = getTileAt(tileStart);
            //TileView tileViewStart = new TileView(tileStart);
            tileStarting.highlight(Color.BLUE);
            Position tileDestination = m.getDestination();
            Tile tileEnding = getTileAt(tileDestination);
            tileEnding.highlight(Color.BLUE);
        }
    }
    ...
    private void secondClick(Tile tile) {
        Position tileStart = startingLocation.getPosition();
        Position tileEnd = tile.getPosition();
        Move q = new Move(tileStart, tileEnd);
        try {
            controller.makeMove(q);
            controller.endTurn();
    ...
    public void updateView(Move moveMade, List<Position> capturedPositions) {
        ...
        for (Tile[] t : tiling) {
            for (Tile tt : t) {
                tt.clear();
                tt.setSymbol(controller.getSymbolForPieceAt(tt.getPosition()));
            }
        }
        getTileAt(moveMade.getDestination()).highlight(Color.GREEN);
    }
    ...
    private PieceType handlePromotion() {
        ChoiceDialog<PieceType> choice = new ChoiceDialog<PieceType>(controller
            .getDefaultPromotionType(), controller.getPromotionTypes());
        return (PieceType) choice.showAndWait().get();
    }
